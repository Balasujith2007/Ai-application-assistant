...
export function InterviewCard({
  ...
  const roundBadgeColors: Record<string, string> = {
    'Technical Interview': 'bg-kit-50 text-kit-700 border-kit-200',
    'HR Interview': 'bg-kit-50 text-kit-700 border-kit-200',
      ...
      <div>
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-center gap-3">
            <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-kit-50 text-kit-600 font-bold text-lg">
              <Building2 className="h-6 w-6" />
            </div>
            <div>
              <h3 className="font-bold text-gray-900 text-lg leading-snug group-hover:text-kit-600 transition-colors">
        ...
        <div className="mt-5 space-y-2 rounded-xl border border-gray-100 bg-gray-50/50 p-3.5 text-xs font-semibold text-gray-700">
          <div className="flex items-center gap-2">
            ...
            <Clock className="h-4 w-4 text-kit-600 shrink-0" />
            <span>{interview.time} ({interview.duration})</span>
          </div>

          <div className="flex items-center gap-2">
            <Video className="h-4 w-4 text-kit-600 shrink-0" />
      ...
      <div className="mt-6 flex items-center justify-between pt-4 border-t border-gray-100 gap-2">
        ...
        {interview.meetingLink && !isCompleted ? (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="inline-flex items-center gap-1.5 rounded-lg bg-kit-600 px-3.5 py-2 text-xs font-semibold text-white shadow-xs hover:bg-kit-700 transition-colors"
          >
