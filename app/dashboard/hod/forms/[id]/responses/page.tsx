...
export default function FormResponsesPage() {
  ...
  const fetchResponses = useCallback(async () => {
    ...
    try {
      ...
      const res = await axios.get(`/api/hod/forms/${formId}/responses`, {
  ...
  const handleExportExcel = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await axios.get(`/api/hod/forms/${formId}/responses?export=excel`, {
        headers: { Authorization: `Bearer ${token}` },
        responseType: 'blob',
      });
      ...
      link.setAttribute('download', `CareerAI_Form_Responses_${safeTitle}_${new Date().toISOString().slice(0, 10)}.xlsx`);
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-12 text-center shadow-2xs space-y-2">
          ...
          <p className="text-xs text-gray-500 max-w-sm mx-auto">
            Once students submit their responses to this form, they will appear here.
          </p>
        </div>
      ) : (
