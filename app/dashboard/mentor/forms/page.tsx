...
export default function MentorFormsPage() {
  ...
  const fetchForms = useCallback(async () => {
    ...
    try {
      ...
      const res = await axios.get('/api/mentor/forms', { headers });
      setForms(res.data.data || []);
    } catch (err: any) {
      console.error('Failed to load mentor forms:', err);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleCreateForm = async () => {
    ...
    try {
      ...
      if (newFormId) {
        router.push(`/dashboard/mentor/forms/${newFormId}/edit`);
      } else {
  ...
  const handlePublish = async (form: FormItem) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.post(
        `/api/mentor/forms/${form.id}/publish`,
        { action: 'publish' },
  ...
  const handleClose = async (form: FormItem) => {
    if (!confirm(`Are you sure you want to close "${form.title}"? Assigned students will no longer be able to submit responses.`)) return;
    setActionId(form.id);
    try {
      const token = localStorage.getItem('token');
      await axios.post(
  ...
  const handleDelete = async (form: FormItem) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`/api/mentor/forms/${form.id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      fetchForms();
    } catch (err: any) {
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-12 text-center shadow-2xs space-y-3">
          ...
          <p className="text-xs text-gray-500 max-w-sm mx-auto">
            Create your first form to collect progress updates, feedback, or custom information from your assigned students.
          </p>
          <button
            onClick={handleCreateForm}
