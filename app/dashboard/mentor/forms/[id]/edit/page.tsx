...
export default function MentorFormBuilderEditorPage() {
  ...
  const fetchFormDetails = useCallback(async () => {
    ...
    try {
      ...
      const res = await axios.get(`/api/mentor/forms/${formId}`, {
  ...
  const handlePublishForm = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.post(
        `/api/mentor/forms/${formId}/publish`,
        { action: 'publish' },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      setFormStatus('PUBLISHED');
      alert('Form published successfully! Assigned students have been notified.');
      router.push('/dashboard/mentor/forms');
    } catch (err: any) {
          ...
          <div className="w-full max-w-3xl rounded-2xl bg-white p-6 shadow-2xl space-y-4 my-8 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between border-b border-gray-100 pb-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-gray-500">How assigned students will see this form</span>
              </div>
